...
//  std::cout << states_ << "\n";
  ...
  if (state.IsChanceNode()) {
    for (const auto &action_prob : state.ChanceOutcomes()) {
      if (save_states_) {
        ...
        InitializeInfostateNodes(*state.Child(action_prob.first), cfr_state);
      }
    }
    return;
  }
  ...
  for (const Action& action : legal_actions) {
    if (save_states_) {
      ...
      InitializeInfostateNodes(*state.Child(action), cfr_state);
    }
  }
}
...
void CFRInfoStateValues::ApplyRegretMatching() {
  ...
  for (int aidx = 0; aidx < num_actions(); ++aidx) {
    if(std::abs(cumulative_regrets[aidx]) < 1e-12) {
...
// Explicit template instantiations for ComputeCounterFactualRegret
template std::vector<double> CFRSolverBase::ComputeCounterFactualRegret<CfrState>(
    CfrState&, const absl::optional<int>&, const std::vector<double>&,
    const std::vector<const Policy*>*);
template std::vector<double> CFRSolverBase::ComputeCounterFactualRegret<const State>(
    const State&, const absl::optional<int>&, const std::vector<double>&,
    const std::vector<const Policy*>*);

// Explicit template instantiations for ComputeCounterFactualRegretForActionProbs
template std::vector<double> CFRSolverBase::ComputeCounterFactualRegretForActionProbs<CfrState>(
    CfrState&, const absl::optional<int>&, const std::vector<double>&, const int,
    const std::vector<double>&, const std::vector<Action>&,
    std::vector<double>*, const std::vector<const Policy*>*);
template std::vector<double> CFRSolverBase::ComputeCounterFactualRegretForActionProbs<const State>(
    const State&, const absl::optional<int>&, const std::vector<double>&, const int,
    const std::vector<double>&, const std::vector<Action>&,
    std::vector<double>*, const std::vector<const Policy*>*);
